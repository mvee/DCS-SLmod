...
do
	...
	slmod.stats.resetStatsFile = function () 
		--slmod.info('in reset stats file')
		if statsF then -- if statsF upvalue already open...
			--slmod.info('closing stats file')
			statsF:close()
			statsF = nil
		end
		local function makeBackup(s)
			local statsBackup = io.open(statsDir .. '\\SlmodStats_BACKUP.lua', 'w')
			if statsBackup then
				statsBackup:write(s)
				statsBackup:close()
				slmod.info('old SlmodStats.lua file backed up to ' .. statsDir .. '\\SlmodStats_BACKUP.lua')
			else
				slmod.error('Unable to create stats backup file, could not open file: ' .. statsDir .. '\\SlmodStats_BACKUP.lua' )
			end
			...
			if prevStatsF then
				...
				if statsFunc then
					...
					if not bool then
						...
						if env.stats then
							stats = env.stats
							slmod.info('using SlmodStats as defined in ' .. statsDir .. 'SlmodStats.lua')
						else
							slmod.error('no stats table in file ' .. statsDir .. 'SlmodStats.lua')
							makeBackup(statsS)
						end
					end
				else
					slmod.error('unable to load SlmodStats, reason: ' .. tostring(err1))
					makeBackup(statsS)
				end
				
			else
				slmod.warning('Unable to open SlmodStats.lua, will make a new SlmodStats.lua file.')	
			end
		end
		if not stats then -- if stats still doesn't exist
			stats = {} 
		end
		--Now, stats should be opened, or if not run, at least backed up..  Now, write over the old stats and return a file handle.
		local newStatsS = slmod.serialize('stats', stats) ..'\n'
		statsF = io.open(statsDir .. '\\SlmodStats.lua', 'w')
	...
	function slmod.stats.onMission()  -- for creating per-mission stats.
		if slmod.config.enable_mission_stats then
			if slmod.config.write_mission_stats_files then
				...
				misStatsF, err = io.open(missionStatsDir .. missionName .. '- ' .. os.date('%b %d, %Y at %H %M %S.lua'), 'w')
...
function slmod.unitIsAlive(unitName)
	...
	function slmod.create_SlmodStatsMenu()
		...
		showVars.description = 'Say "-stats show" for a stats summary for all currently connected players.'
		...
		statsMeVars.description = 'Say "-stats me" for a short summary of your stats.'
		...
		fullStatsMeVars.description = 'Say "-full stats me" for a full description of your stats.'
		...
		statsForIdVars.description = 'Say "-stats id <number>", where "<number>" is a player\'s Stats ID#, to get summarized stats for that player.'
		...
		statsForNameVars.description = 'Say "-stats name <player>", to get summarized stats for that player (Only works for connected clients).'
		...
		fullStatsForIdVars.description = 'Say "-full stats id <number>", where "<number>" is a player\'s Stats ID#, to get FULL stats for that player.'
		...
		---------------------------------------------------------------------------------------------------------------------------------------
		-- seventh item, -full stats for name <player>
		local fullStatsForNameVars = {}
		fullStatsForNameVars.menu = SlmodStatsMenu
		fullStatsForNameVars.description = 'Say "-full stats name <player>" to get FULL stats for that player (Only works for connected clients).'
		...
		if slmod.config.enable_mission_stats then
			...
			statsModeSwitchVars.description = 'Say "-stats mode" to cycle between different stats modes (such as "server" or "mission").'
